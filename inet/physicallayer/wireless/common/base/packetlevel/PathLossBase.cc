...
double PathLossBase::computePathLoss(const ITransmission *transmission, const IArrival *arrival) const
{
    ...
    // Date: 08/06/2022
    // Change the path loss calculation to include comprehensive channel
    double pathLoss = computeCompositePathLoss(propagationSpeed, centerFrequency, distance, transmission, arrival);
    // std::cout << "Path Loss: " << pathLoss << std::endl;
    return pathLoss;
}
