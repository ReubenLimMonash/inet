...
double FreeSpacePathLoss::computeCompositePathLoss(mps propagationSpeed, Hz frequency, m distance, const ITransmission *transmission, const IArrival *arrival) const
{
    m waveLength = propagationSpeed / frequency;
    return computeFreeSpacePathLoss(waveLength, distance, alpha, systemLoss);
}
