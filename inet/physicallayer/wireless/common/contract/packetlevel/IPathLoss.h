...
class INET_API IPathLoss : public IPrintableObject
{
  public:
    ...
    /**
     * Returns the loss factor as a function of propagation speed, carrier
     * frequency and distance, using composite fading model. The value is in the range [0, 1] where 1 means
     * no loss at all and 0 means all power is lost.
     */
    virtual double computeCompositePathLoss(mps propagationSpeed, Hz frequency, m distance, const ITransmission *transmission, const IArrival *arrival) const = 0;
