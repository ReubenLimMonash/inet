...
int Dcaf::requestChannel(IChannelAccess::ICallback *callback)
{
    Enter_Method("requestChannel");
    this->callback = callback;
    int backoffPeriodGen = -1; // Added to record backoff periods
    if (owning) {
        callback->channelGranted(this);
    }
    else if (!contention->isContentionInProgress()) {
        // contention->startContention(cw, ifs, eifs, slotTime, this);
        backoffPeriodGen = contention->startContention(cw, ifs, eifs, slotTime, this);
        // std::cout << "Backoff Period:" << std::endl;
    }
    else {
        EV_DEBUG << "Contention has been already started.\n";
    }
    return backoffPeriodGen;
}
