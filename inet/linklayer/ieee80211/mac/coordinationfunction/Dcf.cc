...
void Dcf::processUpperFrame(Packet *packet, const Ptr<const Ieee80211DataOrMgmtHeader>& header)
{
    ...
    double backoffPeriodGen = 0.0;
    if (!pendingQueue->isEmpty()) {
        EV_DETAIL << "Requesting channel" << endl;
        backoffPeriodGen = (double) channelAccess->requestChannel(this);
    }
    // std::cout << "Backoff Period Gen:" << std::endl;
    // std::cout << backoffPeriodGen << std::endl;
    // std::cout << typeid(backoffPeriodGen).name() << std::endl;
    if (backoffPeriodGen > 0){
        try{
            auto snirInd = packet->addTagIfAbsent<SnirInd>();
            snirInd->setBackoffPeriod(backoffPeriodGen);
        }
        catch(...){
            std::cout << "Failed to record non-zero backoff period!" << std::endl;
            std::cout << "Backoff Period Gen:" << std::endl;
            std::cout << backoffPeriodGen << std::endl;
        }
...
void Dcf::frameSequenceFinished()
{
    ...
    // std::cout << "Entering mode frameSequenceFinished" << std::endl;
    emit(IFrameSequenceHandler::frameSequenceFinishedSignal, frameSequenceHandler->getContext());
    channelAccess->releaseChannel(this);
    int backoffPeriodGen = 0;
    if (hasFrameToTransmit())
        // Try to get next frame
        // auto nextFrame = channelAccess->getInProgressFrames()->getFrameToTransmit();
        // std::cout << channelAccess->getInProgressFrames()->getFrameToTransmit()->getName() << std::endl; 
        backoffPeriodGen = channelAccess->requestChannel(this);
