...
Packet *PacketQueue::pullPacket(cGate *gate)
{
    ...
    // std::cout << "Queueing time: " << std::endl;
    // std::cout << queueingTime.dbl() << std::endl;
    // std::cout << packet->destAddr << std::endl;
    // std::cout << destAddr.str() << std::endl;
    ...
    updateDisplayString();
    // Adding queueing time
    // First, let's check if the packet has a queueing time recorded
    auto prevQueueingTime = 0.0;
    if (packet->findTag<SnirInd>()){
        // If previous queueing time exists, record it down and add to current queueing time to get total queueing time
        prevQueueingTime = packet->getTag<SnirInd>()->getQueueingTime();
        // std::cout << "Prev Queueing time: " << std::endl;
        // std::cout << prevQueueingTime << std::endl;
        if (std::isnan(prevQueueingTime) || (prevQueueingTime < 0)) {prevQueueingTime = 0.0;}
    }
    auto snirInd = packet->addTagIfAbsent<SnirInd>();
    snirInd->setQueueingTime(queueingTime.dbl() + prevQueueingTime);
    return packet;
}
