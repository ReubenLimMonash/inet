    ...
    if (costAttribute != nullptr)
        ...
        if (!strcmp(metric, "hopCount"))
            ...
            if (!isReceptionPossible)
                ...
                const double distance = arrival->getStartPosition().distance(transmission->getStartPosition());
                const IReceptionResult *receptionResult = receiver->computeReceptionResult(listening, reception, interference, snir, receptionDecisions, distance);
